...
%                                  [{0} / 1]
%           -.adi.*                refer to opts.adi in MESS_LRADI for 
%                                  more info  
...
Def.shifts.b0           = ones(sys.n,1);    % only for MESS
Def.shifts.method       = 'heur';           % only for MESS  
Def.shifts.info         = 0;                % only for MESS  
...
%             'prm',speye(size(sys.A)),'type','N','haveE',sys.isDescriptor);
        ...
        [messOpts.shifts.p]=mess_para(eqn,messOpts,oper);

        % low-rank adi
        messOpts.adi=struct('maxiter',Opts.maxiter,'res_tol',Opts.restol,'rel_diff_tol',Opts.rctol,...
            'info',0,'norm',Opts.adi.norm,'LDL_T',Opts.adi.LDL_T);

        Sout = mess_lradi(eqn,messOpts,oper);
        S = Sout.Z;
        ...
        elseif (nargout == 2 && Opts.infoLyap == 0) || nargout == 3 % usage: [S,R] = lyapchol(sys), [S,R,data] = lyapchol(sys)
            if sys.isSym && ~any(size(sys.B)-size(sys.C')) && norm(full(sys.B-sys.C'))==0
                ...
                eqn.type='T';
                Rout = mess_lradi(eqn,messOpts,oper);
                R = Rout.Z;
